...
void IBusBM::loop(void) {
  ...
  if((millis()-RemoteControl.timestamp)>50)
  {
      RemoteControl.RemoteControlSign = 0;  
  }
  else
  {
      RemoteControl.RemoteControlSign = 1;     
  }
  ...
  while (stream->available() > 0) 
  {
    ...
    // 只有当我们在>3ms内没有听到任何消息时才考虑新的数据包 only consider a new data package if we have not heard anything for >3ms
    uint32_t now = millis();
        ...
        if (chksum == (v << 8) + lchksum) {
          ...
          if (buffer[0]==PROTOCOL_COMMAND40) {
            ...
            for (uint8_t i = 1; i < PROTOCOL_CHANNELS * 2 + 1; i += 2) {
              ...
              RemoteControl.timestamp = millis();
            }
            cnt_rec++;
